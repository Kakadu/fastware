...
let lev_dist (s1 : string) (s2 : string) =
    ...
    if i = m then ()
      ...
      let rec go j =
        if j = n then ()
          ...
          let substCost =
            if Char.equal s1char (String.unsafe_get s2 j) then 0 else 1
          in
          v1.(j + 1) <-
            min (substCost + substCostBase) (1 + min delCost insConst);
          go (j + 1)
      in
      go 0;
      loop (i + 1) v1 v0
  ...
  (if m % 2 = 0 then v0Init else v1Init).(n)

let () =
  Stdlib.Printf.printf "%d\n%!" (lev_dist s1 s2);
  Stdlib.Printf.printf "%d\n%!" (lev_dist s1 s3)

let test ~name f arg = (name, f, arg)

let () =
  Benchmark.tabulate
  @@ Benchmark.throughputN ~style:Nil ~repeat:1 1
       [
         test ~name:"leven1" (fun () -> lev_dist s1 s2) ();
         test ~name:"leven1" (fun () -> lev_dist s1 s3) ();
       ]
